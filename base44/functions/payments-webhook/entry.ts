...
async function handleOrderApproved(db: any, eventData: any): Promise<Response> {
  ...
  // ===== APP-SPECIFIC =====
  // Grant access: set the user's `plan` field to the purchased product ID. This unlocks
  // the Starter tier in the app. Idempotent — update is safe to run multiple times.
  ...
  if (grantUserId) {
    ...
    console.log("payments-webhook: no user found to grant — pending claim", { buyerEmail });
  }
  // ===== END APP-SPECIFIC =====
...
async function handleSubscriptionEnded(db: any, eventData: any): Promise<Response> {
  ...
  // ===== APP-SPECIFIC =====
  // Revoke access: clear the user's `plan` field. Idempotent.
  let revokeUserId = purchase.appUserId;
  if (!revokeUserId && purchase.buyerEmail) {
    const users = await db.entities.User.filter({ email: purchase.buyerEmail });
    revokeUserId = users?.[0]?.id;
  }
  if (revokeUserId) {
    await db.entities.User.update(revokeUserId, { plan: null });
